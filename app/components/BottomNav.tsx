import {
  ...
  UserCircle,
  LogOut,
} from "lucide-react"; // Tambah LogOut di sini
...
function isActive(current: Screen, screen: Screen) {
  if (screen === "modules")
    return current === "modules" || current === "module-detail";
  if (screen === "challenge")
    return (
      current === "challenge" ||
      current === "challenge-15day" ||
      current === "portfolio-sim"
    );
  if (screen === "profile")
    return current === "profile" || current === "settings";
...
interface SidebarNavProps {
  ...
  onLogout: () => void; // Tambahkan prop fungsi logout di sini
}
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{
          background: isActive(current, "profile")
            ? "oklch(0.72 0.19 145 / 0.1)"
            : "var(--card)",
          border: `1px solid ${isActive(current, "profile") ? "oklch(0.72 0.19 145 / 0.3)" : "var(--border)"}`,
        }}
      >
            ...
            <button
              ...
              style={{
                background: active
                  ...
                  : "transparent",
                border: active
                  ? "1px solid oklch(0.72 0.19 145 / 0.25)"
      ...
      {/* Bagian Bawah: Tombol Logout & Versi */}
      <div
      ...
      >
        <button
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-left hover:bg-destructive/10 text-destructive/80 hover:text-destructive group"
          style={{ transition: "all 0.2s" }}
        >
          <LogOut
            size={18}
            className="flex-shrink-0 stroke-[1.5] group-hover:stroke-[2]"
          />
